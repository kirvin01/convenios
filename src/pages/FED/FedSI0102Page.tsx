// src/components/FedSI0102Page.tsx
// Reporte FED SI-01_02 — Anemia en gestantes: seguimiento Hb + tratamiento (3 controles)
...
import {
  ...
  type ResumenRow,
  type AnemiaRow,
} from '../../services/servicesFED/fedSI0102Service';
...
// Color institucional del reporte: rojo anemia
const COLOR_PRIMARIO  = '#1565C0';
const COLOR_SECUNDARIO = '#00695C';

// ── Columnas DataGrid — Territorial ──────────────────────────────────────────
...
const columnasDetalleTerritorial: GridColDef[] = [
  ...
  {
    field: 'avance_pct', headerName: 'Avance %', width: 105,
  ...
  { field: 'denominador_apn',              headerName: 'Con APN',       width: 85,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'denominador_apn_hb1',          headerName: 'Hb1',           width: 70,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'denominador_apn_hb1_Dx',       headerName: 'Dx Anemia',     width: 90,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'denominador_apn_Hb1_Dx_trat1', headerName: 'Trat1',         width: 70,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'numerador_Hb2',                headerName: 'Hb2',           width: 70,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'numerador_Hb2_Trat2',          headerName: 'Hb2+T2',        width: 80,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'numerador_Hb3',                headerName: 'Hb3',           width: 70,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'numerador_Hb3_Trat3',          headerName: 'Hb3+T3',        width: 80,  type: 'number', headerAlign: 'center', align: 'center' },
  { field: 'numerador_Hb3_Trat4',          headerName: 'Hb3+T4',        width: 80,  type: 'number', headerAlign: 'center', align: 'center' },
];

// ── Columnas DataGrid — Redes ─────────────────────────────────────────────────
...
// ── Tarjeta de embudo de seguimiento ─────────────────────────────────────────

function EmbудоCard({ label, value, color }: { label: string; value: number | undefined; color: string }) {
  return (
    <Box
      sx={{
        px: 1.5, py: 1, borderRadius: 2, textAlign: 'center',
        border: '1px solid', borderColor: alpha(color, 0.28),
        bgcolor: alpha(color, 0.06), minWidth: 90,
      }}
    >
...
function ProvinciaFila({ prov, distritos, busqueda }: {
  ...
  return (
    <>
      <Box
        ...
        sx={{
          display: 'grid',
          gridTemplateColumns: '40px 1fr 1fr 80px 75px 105px',
          alignItems: 'center', px: 1, py: 0.75,
          bgcolor: alpha(COLOR_PRIMARIO, 0.08), cursor: 'pointer',
          borderBottom: '1px solid', borderColor: alpha(COLOR_PRIMARIO, 0.15),
          '&:hover': { bgcolor: alpha(COLOR_PRIMARIO, 0.13) },
        }}
      >
        <IconButton size="small" sx={{ p: 0 }}>
          {open ? <ArrowDownIcon fontSize="small" /> : <ArrowRightIcon fontSize="small" />}
        </IconButton>
        <Typography variant="body2" fontWeight={700} color="error.dark">{prov.DEPARTAMENTO}</Typography>
        <Typography variant="body2" fontWeight={700} color="error.dark">Total {prov.PROVINCIA}</Typography>
        ...
        {filas.map((d) => (
          <Box
            ...
            sx={{
              ...
              '&:hover': { bgcolor: alpha(COLOR_PRIMARIO, 0.03) },
            }}
          >
            <Box />
...
// ── Sub-componente: Microred colapsable ───────────────────────────────────────

function MicroredFila({ microred, establecimientos, busqueda }: {
  microred: MicroredRow; establecimientos: EstablecimientoRow[]; busqueda: string;
}) {
  const [open, setOpen] = useState(true);
  const filas = useMemo(() => {
    const q = busqueda.toUpperCase();
  ...
  return (
    <>
      <Box
        ...
        sx={{
          display: 'grid',
          gridTemplateColumns: '40px 40px 1fr 80px 75px 105px',
          alignItems: 'center', px: 1, py: 0.6,
          bgcolor: alpha(COLOR_SECUNDARIO, 0.05), cursor: 'pointer',
          borderBottom: '1px solid', borderColor: alpha(COLOR_SECUNDARIO, 0.10),
          '&:hover': { bgcolor: alpha(COLOR_SECUNDARIO, 0.10) },
        }}
      >
        <Box />
        <IconButton size="small" sx={{ p: 0 }}>
        ...
        <Typography variant="body2" fontWeight={600} color="success.dark">{microred.MICRORED}</Typography>
        ...
        {filas.map((e) => (
          <Box
            ...
            sx={{
              ...
              borderBottom: '1px solid', borderColor: alpha('#000', 0.04),
              '&:hover': { bgcolor: alpha(COLOR_SECUNDARIO, 0.04) },
            }}
          >
            <Box /><Box />
...
// ── Sub-componente: Red colapsable ────────────────────────────────────────────

function RedFila({ red, microredes, establecimientos, busqueda }: {
  red: RedRow; microredes: MicroredRow[];
  establecimientos: EstablecimientoRow[]; busqueda: string;
}) {
  const [open, setOpen] = useState(true);
    ...
    <>
      <Box
        ...
        sx={{
          ...
          gridTemplateColumns: '40px 1fr 1fr 80px 75px 105px',
          alignItems: 'center', px: 1, py: 0.75,
          bgcolor: alpha(COLOR_SECUNDARIO, 0.10), cursor: 'pointer',
          borderBottom: '1px solid', borderColor: alpha(COLOR_SECUNDARIO, 0.15),
          '&:hover': { bgcolor: alpha(COLOR_SECUNDARIO, 0.15) },
        }}
      >
        <IconButton size="small" sx={{ p: 0 }}>
          {open ? <ArrowDownIcon fontSize="small" /> : <ArrowRightIcon fontSize="small" />}
...
function GraficoBarras({ titulo, data, color }: {
  titulo: string;
  ...
  return (
    <Paper elevation={0} sx={{
      ...
      <Typography variant="subtitle2" fontWeight={700} color="text.secondary" mb={1.5}>{titulo}</Typography>
      <ResponsiveContainer width="100%" height={210}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: -16, bottom: 4 }}>
          ...
          <Bar dataKey="Denominador" fill={alpha(color, 0.45)} radius={[4,4,0,0]} />
...
function GraficoTendencia({ resumen }: { resumen: ResumenRow[] }) {
  ...
  return (
    <Paper elevation={0} sx={{
      p: 2, borderRadius: 3,
      border: '1px solid', borderColor: alpha(COLOR_PRIMARIO, 0.12),
      boxShadow: `0 4px 24px ${alpha(COLOR_PRIMARIO, 0.08)}`,
    }}>
...
export function FedSI0102Page() {
  ...
  useEffect(() => {
    ...
    fedSI0102Service.getFiltros(ctrl.signal)
      .then((data) => {
        setFiltros(data);
  ...
  const cargarTablaCompleta = useCallback((signal?: AbortSignal) => {
    ...
    fedSI0102Service.getTablaCompleta(
  ...
  const cargarTablaRedes = useCallback((signal?: AbortSignal) => {
    ...
    setLoadingTabla(true);
    setNotification(null);
    fedSI0102Service.getTablaRedes(
  ...
  const datosGrafico = useMemo(() => {
    if (modoVista === 'territorial' && tabla) {
      return tabla.provincias.map((p) => ({
        ...
        Denominador: p.denominador, Numerador: p.numerador, 'Avance %': p.avance_pct,
      }));
    }
    if (modoVista === 'redes' && tablaRedes) {
      return tablaRedes.redes.map((r) => ({
        name: r.RED.length > 11 ? `${r.RED.slice(0, 11)}…` : r.RED,
        Denominador: r.denominador, Numerador: r.numerador, 'Avance %': r.avance_pct,
      }));
    }
    return [];
  }, [modoVista, tabla, tablaRedes]);

  const totalActual = (modoVista === 'territorial' ? tabla?.total : tablaRedes?.total) as AnemiaRow | undefined;
  ...
  const colorModo   = modoVista === 'territorial' ? COLOR_PRIMARIO : COLOR_SECUNDARIO;
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 3, mb: 4 }}>
      ...
      <Box display="flex" alignItems="center" gap={1.5} mb={1}>
        <Avatar sx={{ bgcolor: alpha(COLOR_PRIMARIO, 0.1), color: COLOR_PRIMARIO, width: 44, height: 44 }}>
          <BloodtypeIcon />
        </Avatar>
        <Box>
          <Typography variant="h4" fontWeight={700}>FED SI-01.02</Typography>
          <Typography color="text.secondary">
            Porcentaje de gestantes con anemia diagnosticada en el primer APN que recibieron seguimiento y tratamiento completo (Hb1 → Hb2 → Hb3).
          </Typography>
        </Box>
      </Box>
      <Divider sx={{ mb: 3 }} />
      ...
      <Box display="flex" justifyContent="center" mb={3}>
        <ToggleButtonGroup
          ...
          sx={{
            ...
            borderColor: alpha(colorModo, 0.20), borderRadius: 3,
            overflow: 'hidden', boxShadow: `0 2px 12px ${alpha(colorModo, 0.10)}`,
        ...
        >
          <ToggleButton value="territorial"><AccountTreeIcon fontSize="small" />Organización Territorial</ToggleButton>
          <ToggleButton value="redes"><HubIcon fontSize="small" />Redes Integradas de Salud</ToggleButton>
        </ToggleButtonGroup>
      </Box>
      ...
      {(tabla || tablaRedes) && (
        <>
            ...
            <Grid item xs={12} sm={4} md={3}>
              <Paper elevation={0} sx={{
                ...
                background: modoVista === 'territorial'
                  ? `linear-gradient(135deg, ${COLOR_PRIMARIO} 0%, #1565C0 100%)`
                  : `linear-gradient(135deg, ${COLOR_SECUNDARIO} 0%, #00897B 100%)`,
          ...
          {/* ── Embudo de seguimiento de anemia ── */}
          {totalActual && (
            <Paper elevation={0} sx={{
              p: 2, mb: 3, borderRadius: 3,
              border: '1px solid', borderColor: alpha(COLOR_PRIMARIO, 0.12),
              boxShadow: `0 2px 12px ${alpha(COLOR_PRIMARIO, 0.06)}`,
            }}>
              <Box display="flex" alignItems="center" gap={1} mb={1.5}>
                <TimelineIcon fontSize="small" sx={{ color: COLOR_PRIMARIO }} />
                <Typography variant="subtitle2" fontWeight={700} color="text.secondary">
                  Embudo de seguimiento — {mesActual} {anioActual}
                </Typography>
              </Box>
              <Box display="flex" flexWrap="wrap" gap={1} alignItems="center">
                <EmbудоCard label="Con APN"        value={totalActual.denominador_apn}              color="#1565C0" />
                <Typography variant="body2" color="text.disabled">›</Typography>
                <EmbудоCard label="Hb1 tomada"     value={totalActual.denominador_apn_hb1}          color="#0277BD" />
                <Typography variant="body2" color="text.disabled">›</Typography>
                <EmbудоCard label="Dx Anemia"      value={totalActual.denominador_apn_hb1_Dx}       color={COLOR_PRIMARIO} />
                <Typography variant="body2" color="text.disabled">›</Typography>
                <EmbудоCard label="Trat. 1"        value={totalActual.denominador_apn_Hb1_Dx_trat1} color="#E53935" />
                <Typography variant="body2" color="text.disabled">›</Typography>
                <EmbудоCard label="Hb2 + Trat2"    value={totalActual.numerador_Hb2_Trat2}          color="#F57F17" />
                <Typography variant="body2" color="text.disabled">›</Typography>
                <EmbудоCard label="Hb3 + Trat3"    value={totalActual.numerador_Hb3_Trat3}          color="#2E7D32" />
                <Typography variant="body2" color="text.disabled">›</Typography>
                <EmbудоCard label="Hb3 + Trat4"    value={totalActual.numerador_Hb3_Trat4}          color="#1B5E20" />
              </Box>
            </Paper>
          )}
        </>
      ...
      {!mes ? (
        <Paper elevation={0} sx={{
          ...
          <BloodtypeIcon sx={{ fontSize: 52, color: alpha(colorModo, 0.2), mb: 1 }} />
        ...
        <Box display="flex" justifyContent="center" alignItems="center" mt={6} gap={2}>
          ...
          <Typography color="text.secondary">Cargando datos de anemia…</Typography>
        </Box>
        ...
        <Paper elevation={0} sx={{
          ...
          {/* Cabecera */}
          <Box sx={{
            px: 2.5, py: 1.5, bgcolor: alpha(colorModo, 0.03),
            ...
            <Box display="flex" alignItems="center" gap={1}>
              ...
              <Chip label={`${mesActual} ${anioActual}`} size="small"
                color={modoVista === 'territorial' ? 'error' : 'success'} variant="outlined" />
            </Box>
            <Box display="flex" gap={1}>
              {(['jerarquica', 'plana'] as const).map((v) => (
                <Chip key={v}
                  ...
                  color={vista === v ? (modoVista === 'territorial' ? 'error' : 'success') : 'default'}
                  onClick={() => setVista(v)}
                  sx={{ cursor: 'pointer' }}
                />
              ))}
          ...
          {modoVista === 'territorial' && tabla && (
            <>
              {vista === 'jerarquica' && (
                <>
                  <Box sx={{
                    display: 'grid', gridTemplateColumns: '40px 1fr 1fr 80px 75px 105px',
                    px: 1, py: 1, bgcolor: alpha(COLOR_PRIMARIO, 0.06),
                    borderBottom: '2px solid', borderColor: alpha(COLOR_PRIMARIO, 0.15),
                  }}>
                  ...
                  {tabla.total && (
                    <Box sx={{
                      ...
                      bgcolor: COLOR_PRIMARIO, borderTop: '2px solid', borderColor: '#7F0000',
                    }}>
                      <Box />
              ...
              {vista === 'plana' && (
                <Box sx={{ height: 600 }}>
                  <DataGrid
                    ...
                    sx={{
                      border: 'none',
                      '& .MuiDataGrid-columnHeaders': { bgcolor: alpha(COLOR_PRIMARIO, 0.05), borderBottom: `2px solid ${alpha(COLOR_PRIMARIO, 0.15)}` },
                      '& .MuiDataGrid-columnHeaderTitle': { fontWeight: 700, fontSize: '0.8rem', color: '#1A2332' },
                      '& .MuiDataGrid-row:hover': { bgcolor: alpha(COLOR_PRIMARIO, 0.04) },
                      '& .MuiDataGrid-row:nth-of-type(even)': { bgcolor: alpha(COLOR_PRIMARIO, 0.015) },
                      '& .MuiDataGrid-cell': { borderColor: alpha('#000', 0.05) },
                      '& .MuiDataGrid-footerContainer': { borderTop: `1px solid ${alpha('#000', 0.08)}`, bgcolor: alpha(COLOR_PRIMARIO, 0.02) },
                    }}
                  />
                </Box>
              )}
            ...
            <>
              {vista === 'jerarquica' && (
                <>
                  <Box sx={{
                    display: 'grid', gridTemplateColumns: '40px 1fr 1fr 80px 75px 105px',
                    px: 1, py: 1, bgcolor: alpha(COLOR_SECUNDARIO, 0.06),
                    borderBottom: '2px solid', borderColor: alpha(COLOR_SECUNDARIO, 0.15),
                  }}>
                  ...
                  {tablaRedes.total && (
                    <Box sx={{
                      display: 'grid', gridTemplateColumns: '40px 1fr 1fr 80px 75px 105px',
                      alignItems: 'center', px: 1, py: 1,
                      bgcolor: COLOR_SECUNDARIO, borderTop: '2px solid', borderColor: '#004D40',
                    }}>
                      <Box />
                      <Typography variant="body2" fontWeight={900} color="#fff">TOTAL GENERAL</Typography>
                      <Box />
              ...
              {vista === 'plana' && (
                <Box sx={{ height: 600 }}>
                  <DataGrid
                    ...
                    sx={{
                      border: 'none',
                      '& .MuiDataGrid-columnHeaders': { bgcolor: alpha(COLOR_SECUNDARIO, 0.05), borderBottom: `2px solid ${alpha(COLOR_SECUNDARIO, 0.15)}` },
                      '& .MuiDataGrid-columnHeaderTitle': { fontWeight: 700, fontSize: '0.8rem', color: '#1A2332' },
                      '& .MuiDataGrid-row:hover': { bgcolor: alpha(COLOR_SECUNDARIO, 0.04) },
                      '& .MuiDataGrid-row:nth-of-type(even)': { bgcolor: alpha(COLOR_SECUNDARIO, 0.015) },
                      '& .MuiDataGrid-cell': { borderColor: alpha('#000', 0.05) },
                      '& .MuiDataGrid-footerContainer': { borderTop: `1px solid ${alpha('#000', 0.08)}`, bgcolor: alpha(COLOR_SECUNDARIO, 0.02) },
                    }}
                  />
                </Box>
              )}
