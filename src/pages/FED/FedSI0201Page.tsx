// src/components/FedSI0201Page.tsx
...
import {
  ...
  type ResumenRow,
} from '../../services/servicesFED/fedSI0201Service';
  ...
  return (
    <>
      ...
      >
        ...
        <Typography variant="body2" fontWeight={700} color="primary.dark">{prov.DEPARTAMENTO || 'SIN DPTO'}</Typography>
        <Typography variant="body2" fontWeight={700} color="primary.dark">Total {prov.PROVINCIA || 'SIN PROVINCIA'}</Typography>
      ...
      <Collapse in={open} timeout="auto" unmountOnExit>
        {filas.map((d) => (
          <Box
            key={d.DISTRITO || Math.random().toString()}
            sx={{
              display: 'grid',
              gridTemplateColumns: '40px 1fr 1fr 110px 100px 120px',
          ...
          >
            ...
            <Typography variant="body2" sx={{ pl: 1 }}>{d.DISTRITO || 'SIN DISTRITO'}</Typography>
    ...
    <>
      ...
      >
        ...
        <Typography variant="body2" fontWeight={600} color="success.dark">
          {microred.MICRORED || 'SIN MICRORED'}
        </Typography>
      ...
      <Collapse in={open} timeout="auto" unmountOnExit>
        {filas.map((e) => (
          ...
          >
            <Box /><Box />
            <Typography variant="body2" sx={{ pl: 1, fontSize: '0.8rem' }}>{e.ESTABLECIMIENTO || 'SIN ESTABLECIMIENTO'}</Typography>
...
export function FedSI0201Page() {
  ...
  useEffect(() => {
    ...
    fedSI0201Service.getFiltros(ctrl.signal)
      .then((data) => {
        setFiltros(data);
  ...
  const cargarTablaCompleta = useCallback((signal?: AbortSignal) => {
    ...
    fedSI0201Service.getTablaCompleta(
  ...
  const cargarTablaRedes = useCallback((signal?: AbortSignal) => {
    ...
    setLoadingTabla(true);
    setNotification(null);
    fedSI0201Service.getTablaRedes(
  ...
  useEffect(() => {
    ...
    return () => ctrl.abort();
  }, [modoVista, anio, mes, departamento, provincia, red, microred, cargarTablaCompleta, cargarTablaRedes]);
  ...
  const datosGrafico = useMemo(() => {
    if (modoVista === 'territorial' && tabla) {
      return tabla.provincias.map((p) => {
        const provName = p.PROVINCIA || 'SIN PROVINCIA';
        return {
          name: provName.length > 11 ? `${provName.slice(0, 11)}…` : provName,
    ...
    if (modoVista === 'redes' && tablaRedes) {
      return tablaRedes.redes.map((r) => {
        const redName = r.RED || 'SIN RED';
        return {
          name: redName.length > 11 ? `${redName.slice(0, 11)}…` : redName,
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 3, mb: 4 }}>
      ...
      <Box display="flex" alignItems="center" gap={1.5} mb={1}>
        ...
        <Box>
          <Typography variant="h4" fontWeight={700}>FED SI-02.01</Typography>
          <Typography color="text.secondary">
            Indicador de reporte de nominales (Tamizaje de Hemoglobina y Suplementación con Hierro).
          </Typography>
        </Box>
      </Box>
      <Divider sx={{ mb: 3 }} />
              ...
              <Grid item xs={6} sm={3} md={3}>
                <FormControl fullWidth size="small">
                  ...
                  <Select value={departamento} label="Departamento"
                    ...
                    {filtros?.departamentos.map((d) => <MenuItem key={d || 'vacio'} value={d}>{d || 'SIN DEPARTAMENTO'}</MenuItem>)}
                  </Select>
                </FormControl>
              </Grid>
              <Grid item xs={6} sm={3} md={3}>
                <FormControl fullWidth size="small" disabled={!departamento}>
                  ...
                  <Select value={provincia} label="Provincia"
                    ...
                    {provinciasFiltradas.map((p) => (
                      <MenuItem key={p.provincia || 'vacio'} value={p.provincia}>{p.provincia || 'SIN PROVINCIA'}</MenuItem>
                    ))}
                  </Select>
                </FormControl>
              </Grid>
              ...
              <Grid item xs={6} sm={3} md={3}>
                <FormControl fullWidth size="small">
                  ...
                  <Select value={red} label="Red"
                    ...
                    <MenuItem value="">Todas</MenuItem>
                    {filtros?.redes.map((r) => <MenuItem key={r || 'vacio'} value={r}>{r || 'SIN RED'}</MenuItem>)}
                  </Select>
                </FormControl>
              </Grid>
              <Grid item xs={6} sm={3} md={3}>
                <FormControl fullWidth size="small" disabled={!red}>
                  ...
                  <Select value={microred} label="Microred"
                    ...
                    {microredesFiltradas.map((m) => (
                      <MenuItem key={m.microred || 'vacio'} value={m.microred}>{m.microred || 'SIN MICRORED'}</MenuItem>
                    ))}
                  </Select>
                </FormControl>
              </Grid>
      ...
      {!mes ? (
        ...
        >
          <HealthAndSafetyIcon sx={{ fontSize: 52, color: alpha('#1565C0', 0.2), mb: 1 }} />
                ...
                <>
                  ...
                  <Box sx={{ maxHeight: 520, overflowY: 'auto' }}>
                    {tablaRedes.redes.map((r) => (
                      <RedFila
                        key={r.RED || Math.random().toString()}
                        red={r}
